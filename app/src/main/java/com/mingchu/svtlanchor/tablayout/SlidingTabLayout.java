package com.mingchu.svtlanchor.tablayout;
...
public class SlidingTabLayout extends HorizontalScrollView  {
    ...
    private boolean mIndicatorWidthEqualTitle = true;
...
//        this.mViewPager.addOnPageChangeListener(this);
     ...
     */
    public void setViewPager(String[] titles) {
//        if (vp == null || vp.getAdapter() == null) {
//            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
//        }
//
...
//        mViewPager.setCurrentItem(currentTab, smoothScroll);
     ...
     * 显示未读消息
     *
     * @param position 显示tab位置   位置从0开始
     * @param num      num小于等于0显示红点,num大于0显示数字
     * @param tabCount tablayout的个数
     */
    public void showMsg(int position, int num, int tabCount) {
        ...
        if (tipView != null) {
            ...
            setMsgMargin(position, 4, 2, tabCount);
     ...
     * @param tabCount tablayout的个数
     */
    public void showDot(int position, int tabCount) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        showMsg(position, 0, tabCount);
    }
     ...
     * 位置表示  从0开始
     *
     * @param position      显示的位置从0开始
     * @param leftPadding   左侧padding值
     * @param bottomPadding 底部padding值
     * @param tabCount      显示的tabcount的数量
     */
    public void setMsgMargin(int position, float leftPadding, float bottomPadding, int tabCount) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        ...
        if (tipView != null) {
            ...
            } else if (tabCount == 3) {  //tabcount的数量为3的时候的处理方式
                //一下的这一句是当tab数量多于2个,也就是多个的时候  MsgView的显示位置是正常的
                lp.leftMargin = mTabWidth >= 0 ? (int) (mTabWidth / 2 + textWidth / 2 + dp2px(leftPadding)) :
                        (int) (mTabPadding * 3 / 2 + textWidth + dp2px(leftPadding));
            } else { //当tabcount的数量多于3个的时候的处理方式
